...
uint8_t hw_can_set_speed(struct can_t *can, e_speed_t speed) {
  ...
  int ret = can_init(can->baddr, false, /* TTCM: Time triggered comm mode? */
                     true,  /* ABOM: Automatic bus-off management? */
                     false, /* AWUM: Automatic wakeup mode? */
                     false, /* NART: No automatic retransmission? */
                     false, /* RFLM: Receive FIFO locked mode? */
                     false, /* TXFP: Transmit FIFO priority? */
                     speeds[speed].sjw, speeds[speed].ts1, speeds[speed].ts2,
                     speeds[speed].brp, false, false);
...
uint8_t hw_can_setup(struct can_t *can, e_speed_t speed) {
  ...
  gpio_set_mode(can->rx.port, GPIO_MODE_INPUT, GPIO_CNF_INPUT_PULL_UPDOWN,
                can->rx.pin);
  gpio_set(can->rx.port, can->rx.pin);

  /* Configure CAN pin: TX. */
  gpio_set_mode(can->tx.port, GPIO_MODE_OUTPUT_50_MHZ,
                GPIO_CNF_OUTPUT_ALTFN_PUSHPULL, can->tx.pin);
...
void hw_can_disable(struct can_t *can) {
  ...
  rcc_periph_clock_enable(can->rx.rcc);
  gpio_set_mode(can->rx.port, GPIO_MODE_INPUT, GPIO_CNF_INPUT_FLOAT,
                can->rx.pin);
  rcc_periph_clock_disable(can->rx.rcc);

  rcc_periph_clock_enable(can->tx.rcc);
  gpio_set_mode(can->tx.port, GPIO_MODE_INPUT, GPIO_CNF_INPUT_FLOAT,
                can->tx.pin);
  rcc_periph_clock_disable(can->tx.rcc);
}

uint8_t hw_can_get_msg_nums(can_t *can) { return can->msgs_size; }

uint32_t hw_can_get_pack_nums(struct can_t *can) { return can->nums; }
...
void usb_lp_can_rx0_isr(void) { can_isr(hw_can_get_mscan()); }
...
void exti15_10_isr(void) { exti_reset_request(EXTI11); }
